...
export default function CapturePicker({ availableCaptures, setAvailableTraces, username, repo }: CapturePickerProps): ReactElement {
  ...
  return (
    <>
      ...
      <div>
        ...
        {selectedCaptures.map((capture: captureID, i: number): React.ReactElement => <p key={i}>{capture.folder}/{capture.name}</p>)}
      </div>
    </>
  )
}
